...
export default async function handler(req: NextApiRequest, res: NextApiResponse<TSimpleResponse>) {
  ...
  try {
    ...
    if (!user.displayName) throw Error("Для прохождения карт поля имя, в профиле, должно быть заполнено");

    if (user.uid) {
      const ID = uuid();
      const mapIDV = mapId || uuid();
      await db
        .ref("/maps/" + ID)
        .set({
          userId: user.uid,
        ...
        .then(() => {
          ...
          res
            .status(200)
            .json({ success: true, message: "Карта успешно добавлена! ID карты: " + mapIDV.substring(0, 7) });
        })
        .catch(e => {
          console.log(e);
